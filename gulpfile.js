...
gulp.task('compile', ['clean'], function () {
  return browserify('lib/bespoke-prism.js', { standalone: 'bespoke.plugins.prism' })
    .transform(stringify, {
      appliesTo: { includeExtensions: ['.css'] },
      global: true
    })
    .bundle()
    .pipe(source('bespoke-prism.js'))
    .pipe(buffer())
    .pipe(header([
      '/*!',
      ' * <%= name %> v<%= version %>',
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-prism.min.js'))
    .pipe(uglify())
    .pipe(header([
      '/*! <%= name %> v<%= version %> ',
